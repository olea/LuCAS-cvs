 ...
 *
 * Translated to Spanish by David Martínez Moreno <ender@debian.org>
 *  for NuLies Project (http://nulies.hispalinux.es).
 */
...
static const char menu_instructions[] =
	"Con cursores navega por el menú. "
	"<Enter> selecciona submenús --->. "
	"Las letras marcadas son atajos. "
	"Pulsando <S> incluye, <N> excluye, <M> hace modular. "
	"Presione <Esc><Esc> para salir, <?> para pedir ayuda. "
	"Leyenda: [*] incluído, [ ] excluído, <M> módulo, < > puede ser modular.",
...
inputbox_instructions_int[] =
	...
	"Use la tecla <Tab> para moverse del campo de datos a los botones que hay debajo de él.",
...
inputbox_instructions_string[] =
	...
	"Use la tecla <Tab> para moverse del campo de datos a los botones que hay debajo de él.",
setmod_text[] =
	"Esta opción depende de otra que ha sido configurada como modular.\n"
	"Por ello, esta opción también se construirá como módulo.",
nohelp_text[] =
	"No hay ayuda asociada a esta opción del núcleo.\n",
load_config_text[] =
...
static void conf(struct menu *menu)
{
	...
	while (1) {
		...
		if (menu == &rootmenu) {
			...
			cprint("--- ");
			cprint("L");
			cprint("    Cargar el fichero de una configuración alternativa");
			cprint("S");
			cprint("    Guardar la configuración en un fichero alternativo");
		}
		stat = exec_conf();
		if (stat < 0)
			continue;
	...
	while (1) {
		...
		case 0:
			if (sym_set_string_value(menu->sym, input_buf))
				return;
			show_textbox(NULL, "Ha introducido un valor inválido.", 5, 43);
			break;
		case 1:
			show_help(menu);
			break;
	...
	while (1) {
		...
		case 0:
			...
			if (!conf_read(input_buf))
				return;
			show_textbox(NULL, "¡El fichero no existe!", 5, 38);
			break;
		case 1:
...
int main(int ac, char **av)
{
	...
	sym_calc_value(sym);
	sprintf(menu_backtitle, "Configuración del núcleo Linux v%s.",
	...
	do {
		cprint_init();
		cprint("--yesno");
		cprint(" ¿Desea grabar su nueva configuración para el núcleo?");
		cprint("5");
		cprint("60");
		stat = exec_conf();
	} while (stat < 0);
	...
	if (stat == 0) {
		if (conf_write(NULL)) {
			fprintf(stderr, "\n\n"
				"Error en la escritura de la configuración del núcleo.\n"
				"Los cambios en la configuración del núcleo NO se han guardado."
				"\n\n");
			return 1;
		}
		printf("\n\n"
			"*** Fin de la configuración del núcleo Linux.\n"
			"*** Ejecute «make» para construir el núcleo, o intente «make help» para ayuda."
			"\n\n");
	} else {
		fprintf(stderr, "\n\n"
			"Los cambios en la configuración del núcleo NO se han guardado."
