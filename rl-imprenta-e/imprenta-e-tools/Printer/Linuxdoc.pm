...
# versión sgml así como los métodos necesarios para 
...
sub validate {
	...
	while (<FILE>) {
		if ($_ =~ /(<!DOCTYPE LINUXDOC SYSTEM>)/i) {
			close FILE;
			$valid = 0;
			return $valid;
		}
...
sub convertToPS {
	...
	system ("sgml2latex -l es -c latin -o ps $self->{fileName}");	
}
...
sub convertToPDF {
	...
	if (-f "$file.ps") {
		...
		system ("sgml2latex -l es -c latin -o ps $file");	
		system ("ps2pdf $file.ps");
		system ("rm $file.ps")
	}
...
sub convertToHTML {
	...
	system ("mkdir $self->{name}");	# Crea directorio para el html
	
	system ("cp $self->{fileName} $self->{name}"); # Copia las fuentes
	chdir ($self->{name});
	system ("sgml2html -l es -c latin -s 0 $self->{fileName}");	
	system ("rm $self->{fileName}"); # Elimina las fuentes
